...
// body larger than both maxCacheBodyBytes (1 MiB) and maxDiffBodyBytes
// (10 MiB) is served to the caller in full (no truncation) and is NOT stored
// in the cache. The larger size guards the PR-files fallback, whose JSON page
// ceiling is 20 MiB. A subsequent GET must NOT send If-None-Match (because
// nothing was cached), and the server delivers a fresh response.
func TestETagCaching_LargeBodyServedFullButNotCached(t *testing.T) {
	// Build a body strictly larger than the direct-diff ceiling but below the
	// dedicated PR-files page ceiling.
	const tenMiB = 10 * 1024 * 1024
	largeBody := make([]byte, tenMiB+1)
