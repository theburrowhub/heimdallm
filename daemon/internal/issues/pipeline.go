...
// PRDescriptionResult is the JSON schema the LLM returns when asked to
// generate a PR title and body from the committed diff.
...
type RunOptions struct {
	...
	// produce a richer PR title and body from the diff. When false (default),
	// the pipeline uses the static template strings for backwards compat.
...
func (p *Pipeline) runAutoImplement(ctx context.Context, issue *github.Issue, issueID int64, workDir string, opts RunOptions) (*store.IssueReview, error) {
	...
	if opts.GeneratePRDescription {
		if desc, ok := p.generatePRDescription(ctx, workDir, cli, issue, opts); ok {
...
// generatePRDescription asks the LLM to produce a PR title and body from the
// committed diff. Returns the parsed result and true on success. On any
// failure (diff, LLM call, JSON parse) it logs a warning and returns false
// so the caller falls back to the static template.
func (p *Pipeline) generatePRDescription(ctx context.Context, workDir, cli string, issue *github.Issue, opts RunOptions) (PRDescriptionResult, bool) {
	diff, err := p.git.DiffHead(ctx, workDir)
	if err != nil {
		slog.Warn("issues pipeline: diff for PR description failed, using template",
			"repo", issue.Repo, "number", issue.Number, "err", err)
		return PRDescriptionResult{}, false
	}

	prompt := BuildPRDescriptionPrompt(issue.Number, issue.Title, diff)
	descOpts := executor.ExecOptions{Model: opts.ExecOpts.Model}
	raw, err := p.executor.ExecuteRaw(cli, prompt, descOpts)
	if err != nil {
		slog.Warn("issues pipeline: LLM PR description call failed, using template",
			"repo", issue.Repo, "number", issue.Number, "err", err)
		return PRDescriptionResult{}, false
	}

	var desc PRDescriptionResult
	if err := json.Unmarshal(executor.StripToJSON(raw), &desc); err != nil {
		slog.Warn("issues pipeline: failed to parse PR description JSON, using template",
			"repo", issue.Repo, "number", issue.Number, "err", err)
		return PRDescriptionResult{}, false
	}

	if desc.Title == "" || desc.Body == "" {
		slog.Warn("issues pipeline: LLM returned empty PR title or body, using template",
			"repo", issue.Repo, "number", issue.Number)
		return PRDescriptionResult{}, false
	}

	slog.Info("issues pipeline: LLM-generated PR description",
		"repo", issue.Repo, "number", issue.Number, "title", desc.Title)
	return desc, true
}
