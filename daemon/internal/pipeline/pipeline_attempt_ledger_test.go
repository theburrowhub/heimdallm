...
// The commit-anchored pair the isolation refactor added to the client
// interface. This double predates it; delegate so the ledger assertions keep
// exercising the same behaviour they were written for.
func (a *attemptGH) FetchDiffForCommit(repo string, number int, _ string) (string, error) {
	return a.FetchDiff(repo, number)
}

func (a *attemptGH) SubmitReviewForCommit(repo string, number int, body, event, _ string) (int64, string, error) {
	return a.SubmitReview(repo, number, body, event)
}
