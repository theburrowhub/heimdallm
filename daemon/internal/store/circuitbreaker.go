...
type CircuitBreakerLimits struct {
	PerPR24h  int // max completed reviews per PR HEAD SHA in any 24h window
	PerRepoHr int // max completed reviews per repo in any 1h window
}
...
//
// Only completed reviews persisted in SQLite count. Failed executor runs do not
// produce a review and therefore must not consume the quota or prevent a later
// retry from reviewing the PR.
