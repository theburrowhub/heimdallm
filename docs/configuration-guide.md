...
> **Reload behaviour:** Every field except the adaptive `min_interval`/`max_interval` bounds takes effect on the next `PUT /config` or file reload, without a restart. That includes `use_etag`, `use_graphql` and `rate_limit_safety_threshold`, which are re-applied to the live GitHub client and rate limiter on each reload, and `tier3_interval`, whose ticker is reset in place. `min_interval`/`max_interval` are fixed when the adaptive scheduler is constructed so per-repo back-off state survives a reload; changing those two still needs a daemon restart.
